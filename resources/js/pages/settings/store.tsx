...
export default function StoreSettings({ stores, activeStore, status, message, whatsappSupportUrl }: StoreProps) {
    const [activeTab, setActiveTab] = useState<'general' | 'social' | 'images' | 'services' | 'hours' | 'design' | 'whatsapp'>('general');
    ...
    const [selectedCatalogPhone, setSelectedCatalogPhone] = useState('');
    const [syncingFull, setSyncingFull] = useState(false);

    useEffect(() => {
        if (activeStore && activeStore.whatsapp_access_token && !activeStore.whatsapp_catalog_id) {
            setLoadingCatalogs(true);
            setCatalogError(null);
            
            fetch(`/dashboard/settings/whatsapp/catalogs?store_id=${activeStore.id}`)
                .then(res => {
                ...
                .then(data => {
                    ...
                    if (data.businesses && data.businesses.length > 0 && data.businesses[0].catalogs && data.businesses[0].catalogs.length > 0) {
                        setSelectedCatalogId(data.businesses[0].catalogs[0].id);
                    }
                })
                .catch(err => {
                    console.error(err);
                    setCatalogError('No se pudieron obtener los catálogos de Meta. Verifica que tu cuenta de Facebook tenga los permisos correspondientes.');
                })
                .finally(() => {
                    setLoadingCatalogs(false);
                });
        }
    }, [activeStore?.id, activeStore?.whatsapp_access_token, activeStore?.whatsapp_catalog_id]);

    const handleSelectCatalog = (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedCatalogId) return;

        router.post('/dashboard/settings/whatsapp/select-catalog', {
            store_id: activeStore.id,
            whatsapp_catalog_id: selectedCatalogId,
            whatsapp_catalog_phone: selectedCatalogPhone,
        });
    };

    const handleDisconnect = () => {
        if (confirm('¿Estás seguro de que deseas desvincular tu catálogo de WhatsApp? Se detendrá la sincronización automática.')) {
            router.post('/dashboard/settings/whatsapp/disconnect', {
                store_id: activeStore.id
            });
        }
    };
    ...
    const handleSyncFull = () => {
        setSyncingFull(true);
        router.post('/dashboard/settings/whatsapp/sync-full', {
            store_id: activeStore.id
        }, {
            onFinish: () => setSyncingFull(false)
        });
    };
            ...
            <div className="px-6 py-6 max-w-7xl mx-auto">
                <div className="space-y-6 bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-slate-200/80 dark:border-zinc-800 shadow-sm">
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        {activeTab === 'whatsapp' && (
                            <div className="space-y-6">
                                <div className="p-4 bg-slate-50 dark:bg-zinc-800/40 rounded-xl border border-slate-200 dark:border-zinc-800/80">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-zinc-100 flex items-center gap-2 mb-2">
                                        <Share2 className="h-4 w-4 text-green-500" />
                                        Módulo de Catálogo de WhatsApp Business
                                    </h3>
                                    <p className="text-xs text-slate-500 dark:text-zinc-400 leading-relaxed mb-4">
                                        Este módulo permite que el stock de tus vehículos se sincronice de forma automática en segundo plano con tu catálogo de WhatsApp Business. 
                                        Cada vez que crees, edites o elimines un vehículo, los cambios se reflejarán sin que tengas que hacer nada manual.
                                    </p>

                                    {/* STATE 1: Disconnected from Meta */}
                                    {!activeStore?.whatsapp_access_token && (
                                        <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 space-y-3">
                                            <div className="flex items-center gap-2">
                                                <span className="h-2.5 w-2.5 rounded-full bg-amber-500"></span>
                                                <span className="text-xs font-bold text-amber-700 dark:text-amber-400">Módulo de WhatsApp Desvinculado</span>
                                            </div>
                                            <p className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed">
                                                Para comenzar la sincronización en segundo plano, debes conectar tu cuenta comercial de Meta (Facebook Login for Business). Esto le otorgará permisos a nuestra aplicación para gestionar tu catálogo.
                                            </p>
                                            <div className="pt-2">
                                                <a 
                                                    href={`/dashboard/settings/whatsapp/connect?store_id=${activeStore.id}`}
                                                    className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2.5 rounded-lg shadow-sm text-xs transition-colors"
                                                >
                                    ...
                                    {/* STATE 2: Connected to Meta, but Catalog NOT selected */}
                                    {activeStore?.whatsapp_access_token && !activeStore?.whatsapp_catalog_id && (
                                        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 space-y-4">
                                            <div className="flex items-center gap-2">
                                                <span className="h-2.5 w-2.5 rounded-full bg-blue-500"></span>
                                                <span className="text-xs font-bold text-blue-700 dark:text-blue-400">Meta Conectado - Pendiente Selección de Catálogo</span>
                                            </div>
                                            
                                            {loadingCatalogs ? (
                                                <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-zinc-400">
                                                    <span className="animate-spin h-3.5 w-3.5 border-2 border-slate-500 border-t-transparent rounded-full"></span>
                                                    <span>Obteniendo catálogos de Meta...</span>
                                                </div>
                                            ) : catalogError ? (
                                                <div className="space-y-3">
                                                    <p className="text-xs text-red-600 dark:text-red-400 font-medium">{catalogError}</p>
                                                    <button 
                                                        type="button"
                                                ...
                                                <div className="space-y-3">
                                                    <p className="text-xs text-slate-600 dark:text-zinc-300">
                                                        No encontramos ningún negocio o catálogo en tu cuenta de Meta. Asegúrate de haber creado una cuenta comercial y un catálogo en Meta for Developers.
                                                    </p>
                                                    <button 
                                                        type="button"
                                                ...
                                                <div className="space-y-4">
                                                    <div className="space-y-1.5">
                                                        <Label htmlFor="catalog_select" className="text-xs font-semibold text-slate-700 dark:text-zinc-300">
                                                            Selecciona tu Catálogo de Productos
                                                        </Label>
                                                        <select
                                                            id="catalog_select"
                                                            value={selectedCatalogId}
                                                            onChange={(e) => setSelectedCatalogId(e.target.value)}
                                                            className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg px-3 py-2 text-xs text-slate-800 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                            required
                                                        >
                                                            <option value="" disabled>Selecciona un catálogo...</option>
                                                            {businesses.map((biz) => (
                                                                <optgroup key={biz.business_id} label={biz.business_name}>
                                                                    {biz.catalogs.map((cat: any) => (
                                                    ...
                                                    <div className="space-y-1.5">
                                                        <Label htmlFor="catalog_phone" className="text-xs font-semibold text-slate-700 dark:text-zinc-300">
                                                            Número de Teléfono del Catálogo (Opcional)
                                                        </Label>
                                                        <Input
                                                            id="catalog_phone"
                                                            value={selectedCatalogPhone}
                                                            onChange={(e) => setSelectedCatalogPhone(e.target.value)}
                                                            placeholder="Ej: +54 9 11 1234 5678"
                                                            className="text-xs"
                                                        />
                                                        <span className="text-[10px] text-slate-400 block">
                                                            El número que tus clientes verán vinculado al catálogo en WhatsApp.
                                                        </span>
                                                    </div>

                                                    <div className="flex items-center gap-3 pt-2">
                                                        <Button type="button" onClick={handleSelectCatalog} size="sm" className="text-xs">
                                                            Vincular Catálogo
                                                        </Button>
                                                        <button 
                                                            type="button"
                                    ...
                                    {/* STATE 3: Fully Connected & Active */}
                                    {activeStore?.whatsapp_access_token && activeStore?.whatsapp_catalog_id && (
                                        <div className="space-y-4">
                                            <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4 space-y-3">
                                                <div className="flex items-center gap-2">
                                                    <span className="h-2.5 w-2.5 rounded-full bg-green-500 animate-pulse"></span>
                                                    <span className="text-xs font-bold text-green-700 dark:text-green-400">Módulo WhatsApp Habilitado</span>
                                                </div>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                                    <div>
                                                        <span className="text-slate-400 block font-medium">ID del Catálogo:</span>
                                                        <code className="text-slate-800 dark:text-zinc-200 font-mono font-semibold">{activeStore.whatsapp_catalog_id}</code>
                                                    </div>
                                                    <div>
                                                        <span className="text-slate-400 block font-medium">Número Vinculado:</span>
                                                        <span className="text-slate-800 dark:text-zinc-200 font-semibold">{activeStore.whatsapp_catalog_phone || 'No especificado'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 pt-2">
                                                <Button 
                                                    ...
                                                    variant="outline"
                                                    size="sm"
                                                    className="text-xs border-green-600 text-green-600 hover:bg-green-50 dark:hover:bg-green-950/20"
                                                >
                                                    {syncingFull ? (
                                                        <>
                                                            <span className="animate-spin h-3.5 w-3.5 border-2 border-green-600 border-t-transparent rounded-full mr-2"></span>
                                                            Sincronizando Stock...
                                                        </>
                                                    ) : (
